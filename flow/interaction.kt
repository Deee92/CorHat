...
val GetAvailability : State = state(Interaction) {
    onEntry {
        furhat.ask("${c}, Alright, when are you available for doing the test?")
    }
...
val ChooseCenter : State = state(Interaction) {
    onEntry {
        ...
        furhat.say("You are welcomed to do your test in ${Available_centers(b?.text,c?.text).optionsToText()} on ${b}")
